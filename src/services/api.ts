...
class ApiService {
  ...
  // Job endpoints
  static async getJobs(filters?: { category?: string; location?: string; search?: string }) {
  ...
  static async acceptApplicant(jobId: string, applicationId: string) {
    const response = await fetch(`${API_BASE_URL}/jobs/${jobId}/accept/${applicationId}`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
    });
    return this.handleResponse(response);
  ...
  static async rejectApplicant(jobId: string, applicationId: string) {
    const response = await fetch(`${API_BASE_URL}/jobs/${jobId}/reject/${applicationId}`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
    });
    return this.handleResponse(response);
